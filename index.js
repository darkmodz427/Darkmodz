...
const { Darkmenu } = require('./database/Darkmenu')
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'FN:Dark\n' //NAMA LU
            + 'ORG:Owner Bot;\n' // NAMA CONTACT OWNER
            + 'TEL;type=CELL;type=VOICE;waid=5521980815830:+55 21 98081-5830\n' // NOMER OWNER
            + 'END:VCARD'
prefix = '#'//PREFIX BOT
blocked = ['5521980815830']//NOMER BLOCK
limitawal = '50'
cr = '*BOT WHATSAPP*'
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'DARK'//Nama Bot
                        const ownerName = 'Dark<3'//Nama Owner
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, exemplo ${prefix}marvellogo Dark Criador`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'neonime':
					...
					break  
					case 'bpink':
                  if (args.length < 1) return reply(`Entrada de texto\Exemplo : ${prefix}Dark Bot`)
           ...
           break
				...
				case 'remover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Tchauzinho fdp : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
